...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    ...
    it('should set correct state variables', async () => {
        this.chef = await MasterChef.new(this.deca.address, dev, '1000', '0', '1000', { from: alice });
        await this.deca.transferOwnership(this.chef.address, { from: alice });
        const deca = await this.chef.deca();
        const devaddr = await this.chef.devaddr();
        const owner = await this.deca.owner();
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await MasterChef.new(this.deca.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should give out SUSHIs only after farming time', async () => {
            ...
            assert.equal((await this.deca.totalSupply()).valueOf(), '5500');
        });

        it('should not distribute SUSHIs if no one deposit', async () => {
            // 100 per block farming rate starting at block 200 with bonus until block 1000
            this.chef = await MasterChef.new(this.deca.address, dev, '100', '200', '1000', { from: alice });
            await this.deca.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
            await time.advanceBlockTo('199');
            assert.equal((await this.deca.totalSupply()).valueOf(), '0');
            ...
            assert.equal((await this.deca.totalSupply()).valueOf(), '11000');
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            this.chef = await MasterChef.new(this.deca.address, dev, '100', '300', '1000', { from: alice });
            await this.deca.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            assert.equal((await this.deca.totalSupply()).valueOf(), '22000');
            assert.equal((await this.deca.balanceOf(alice)).valueOf(), '5666');
            ...
            assert.equal((await this.deca.totalSupply()).valueOf(), '55000');
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await MasterChef.new(this.deca.address, dev, '100', '400', '1000', { from: alice });
            await this.deca.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            this.chef = await MasterChef.new(this.deca.address, dev, '100', '500', '600', { from: alice });
            await this.deca.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
