...
contract('DecaLMining', ([alice, bob, carol, dev, minter]) => {
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should give out SUSHIs only after farming time', async () => {
            ...
            assert.equal((await this.token.balanceOf(bob)).valueOf(), '1000');
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            assert.equal((await this.token.balanceOf(this.chef.address)).valueOf(), '4334');
            assert.equal((await this.token.balanceOf(dev)).valueOf(), '1000');
            ...
            assert.equal((await this.token.balanceOf(carol)).valueOf(), '0');
            assert.equal((await this.token.balanceOf(this.chef.address)).valueOf(), '8144');
            assert.equal((await this.token.balanceOf(dev)).valueOf(), '2000');
            ...
            assert.equal((await this.token.balanceOf(bob)).valueOf(), '11831');
            // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
            assert.equal((await this.token.balanceOf(carol)).valueOf(), '26568');
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            assert.equal((await this.token.balanceOf(alice)).valueOf(), '10600');
        });
    });
});
