...
contract('DecaMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        ...
        this.sushi = await DecaToken.new({ from: alice });
        ...
        this.maker = await DecaMaker.new(this.factory.address, bar, this.sushi.address, this.weth.address);
        this.decaWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.sushi.address)).logs[0].args.pair);
    ...
    it('should make SUSHIs successfully', async () => {
        ...
        await this.sushi.transfer(this.decaWETH.address, '10000000', { from: minter });
        ...
        await this.sushi.transfer(this.decaWETH.address, '100000', { from: minter });
